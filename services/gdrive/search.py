...
def search_google_drive_files_ranked_v6(search_string):
    """
    Búsqueda con ranking de relevancia y extracción de textos coincidentes
    1) Primero busca exactamente el string ingresado y da el ranking mayor
    2) Luego usa search_string.split() para búsqueda más amplia
...
# Registrar Unaccent para campos de texto
CharField.register_lookup(Unaccent)
TextField.register_lookup(Unaccent)

def search_google_drive_files_ranked_v7(search_string):
    """
    Búsqueda con ranking de relevancia y extracción de textos coincidentes
    1) Primero busca exactamente el string ingresado y da el ranking mayor
    2) Luego usa search_string.split() para búsqueda más amplia
    Incluye búsqueda flexible en el nombre del archivo (ignora acentos)
    RANKING: 1) Nombre archivo, 2) Nombre folder parent, 3) Todo lo demás
    """
    # Dividir la búsqueda en términos y filtrar palabras de menos de 3 caracteres
    raw_terms = search_string.strip().split()
    ...
    # Construir queries para búsqueda en texto
    # Búsqueda en nombre del archivo - CON UNACCENT (ignora acentos)
    name_queries = Q()
    for term in search_terms:
        # Buscar con el término original (con acentos)
        name_queries |= Q(name__icontains=term)
        # Buscar con UNACCENT (ignora acentos)
        name_queries |= Q(name__unaccent__icontains=term)
        # Buscar con el término normalizado (sin acentos)
        term_normalized = normalize_for_db(term)
        name_queries |= Q(name__icontains=term_normalized)
    
    # Búsqueda en nombre del padre - CON UNACCENT (ignora acentos)
    parent_name_queries = Q()
    for term in search_terms:
        # Buscar con el término original (con acentos)
        parent_name_queries |= Q(parent_drive_file_id__name__icontains=term)
        # Buscar con UNACCENT (ignora acentos)
        parent_name_queries |= Q(parent_drive_file_id__name__unaccent__icontains=term)
        # Buscar con el término normalizado (sin acentos)
        term_normalized = normalize_for_db(term)
        parent_name_queries |= Q(parent_drive_file_id__name__icontains=term_normalized)
    ...
    files = GoogleDriveFile.objects.filter(
        ...
        # Coincidencia con UNACCENT en nombre (ignora acentos)
        name_unaccent_rank=Case(
            When(name__unaccent__icontains=search_string, then=Value(75.0)),
            default=Value(0.0),
            output_field=FloatField()
        ),
        # Coincidencia normalizada en nombre (sin acentos)
        ...
        # Coincidencia parcial de términos en nombre
        name_partial_rank=Case(
            When(name__unaccent__icontains=search_terms[0] if search_terms else "", then=Value(50.0)),
            default=Value(0.0),
            output_field=FloatField()
        ),
        # SearchRank para nombre
        ...
        # Coincidencia con UNACCENT en parent (ignora acentos)
        parent_unaccent_rank=Case(
            When(parent_drive_file_id__name__unaccent__icontains=search_string, then=Value(40.0)),
            default=Value(0.0),
            output_field=FloatField()
        ),
        # Coincidencia normalizada en parent (sin acentos)
        ...
        # Coincidencia parcial de términos en parent
        parent_partial_rank=Case(
            When(parent_drive_file_id__name__unaccent__icontains=search_terms[0] if search_terms else "", then=Value(20.0)),
            default=Value(0.0),
            output_field=FloatField()
        ),
        # SearchRank para parent_name
        ...
        total_rank=(
            # -------- 1) NOMBRE ARCHIVO (PESO MUY ALTO) --------
            Coalesce('exact_name_rank', Value(0.0, output_field=FloatField())) * 5.0 +
            Coalesce('name_unaccent_rank', Value(0.0, output_field=FloatField())) * 4.5 +
            Coalesce('name_normalized_rank', Value(0.0, output_field=FloatField())) * 4.0 +
            Coalesce('name_partial_rank', Value(0.0, output_field=FloatField())) * 3.0 +
            (Coalesce('name_rank', Value(0.0, output_field=FloatField())) * 3.0) +
            # -------- 2) NOMBRE FOLDER PARENT (PESO ALTO) --------
            Coalesce('exact_parent_rank', Value(0.0, output_field=FloatField())) * 2.0 +
            Coalesce('parent_unaccent_rank', Value(0.0, output_field=FloatField())) * 1.8 +
            Coalesce('parent_normalized_rank', Value(0.0, output_field=FloatField())) * 1.5 +
            Coalesce('parent_partial_rank', Value(0.0, output_field=FloatField())) * 1.0 +
            (Coalesce('parent_rank', Value(0.0, output_field=FloatField())) * 1.0) +
            # -------- 3) CONTENIDO Y DESCRIPCIÓN (PESO BAJO) --------
    ...
    def extract_matches_flexible(text, terms, exact_string=None):
        ...
        # 3) FINALMENTE buscar términos individuales (sin boundaries)
        for term in terms:
            if term == exact_string:
                continue
    ...
    # Función para extraer coincidencias de parent_name (folders)
    def extract_parent_matches(text, terms, exact_string=None):
        """
        Extrae fragmentos de texto de parent_name (folders)
        Usa word boundaries para palabras completas
        ...
        # 3) Si no hay coincidencias con word boundaries, buscar coincidencia parcial
        if not matches:
            for term in terms:
                if term == exact_string:
                    continue
    ...
    def extract_all_matches(file, terms, exact_string):
        ...
        # Extraer del nombre del archivo - Búsqueda FLEXIBLE (parcial) SIN boundaries
        if file.name:
            all_matches['name'] = extract_matches_flexible(file.name, terms, exact_string)
            for match in all_matches['name']:
                match['field'] = 'name'
        
        # Extraer del nombre del padre - Búsqueda CON boundaries
        if file.parent_drive_file_id and file.parent_drive_file_id.name:
            all_matches['parent_name'] = extract_parent_matches(file.parent_drive_file_id.name, terms, exact_string)
            for match in all_matches['parent_name']:
                match['field'] = 'parent_name'
        
        # Extraer del contenido de texto
        if hasattr(file, 'document') and file.document and file.document.text_content:
            full_text = file.document.text_content
            all_matches['text_content'] = extract_matches(full_text, terms, exact_string)
            for match in all_matches['text_content']:
    ...
    for file in files:
        ...
        # Si no coincide en nombre, ni en parent, ni en otros campos, saltar
        if not exact_match_in_name and not exact_normalized_match and not name_matches and not parent_matches:
            has_other_match = False
            if hasattr(file, 'document') and file.document:
                if has_search_vector_match(file, search_query) or has_description_vector_match(file, search_query):
        ...
        result = {
            ...
            'relevance_details': {
                'exact_name_rank': float(file.exact_name_rank) if file.exact_name_rank else 0,
                'name_unaccent_rank': float(file.name_unaccent_rank) if hasattr(file, 'name_unaccent_rank') else 0,
                'name_normalized_rank': float(file.name_normalized_rank) if hasattr(file, 'name_normalized_rank') else 0,
                'name_partial_rank': float(file.name_partial_rank) if hasattr(file, 'name_partial_rank') else 0,
                'name_rank': float(file.name_rank) if file.name_rank else 0,
                'exact_parent_rank': float(file.exact_parent_rank) if file.exact_parent_rank else 0,
                'parent_unaccent_rank': float(file.parent_unaccent_rank) if hasattr(file, 'parent_unaccent_rank') else 0,
                'parent_normalized_rank': float(file.parent_normalized_rank) if hasattr(file, 'parent_normalized_rank') else 0,
                'parent_partial_rank': float(file.parent_partial_rank) if hasattr(file, 'parent_partial_rank') else 0,
                'parent_rank': float(file.parent_rank) if file.parent_rank else 0,
                'search_vector_rank': float(file.search_vector_rank) if file.search_vector_rank else 0,
...
def search_google_drive_files_ranked_v8(search_string):
    """
    Búsqueda con ranking de relevancia y extracción de textos coincidentes
    1) Primero busca exactamente el string ingresado y da el ranking mayor
    2) Luego usa search_string.split() para búsqueda más amplia
    Incluye búsqueda flexible en el nombre del archivo (ignora acentos)
    Incluye búsqueda flexible en CONTENIDO y DESCRIPCIÓN (ignora acentos)
    RANKING: 1) Nombre archivo, 2) Nombre folder parent, 3) Todo lo demás
    """
    # Dividir la búsqueda en términos y filtrar palabras de menos de 3 caracteres
    raw_terms = search_string.strip().split()
    ...
    # Búsqueda en contenido de texto - CON UNACCENT (ignora acentos)
    content_queries = Q()
    for term in search_terms:
        content_queries |= Q(document__text_content__icontains=term)
        content_queries |= Q(document__text_content__unaccent__icontains=term)
        term_normalized = normalize_for_db(term)
        content_queries |= Q(document__text_content__icontains=term_normalized)
    
    # Búsqueda en descripción - CON UNACCENT (ignora acentos)
    description_queries = Q()
    for term in search_terms:
        description_queries |= Q(document__description__icontains=term)
        description_queries |= Q(document__description__unaccent__icontains=term)
        term_normalized = normalize_for_db(term)
        description_queries |= Q(document__description__icontains=term_normalized)
    ...
    def extract_matches(text, terms, exact_string=None):
        ...
        # 3) FINALMENTE buscar coincidencias parciales (sin boundaries) - para ignorar acentos
        # Esto permite encontrar "canquen" en "canquén"
        if not matches:
            for term in terms:
                if term == exact_string:
                    continue
                ...
                if term_normalized in text_normalized:
                    ...
                    if pos not in processed_positions:
                        ...
                        highlighted_fragment = fragment.replace(
                            text[pos-context_start:pos-context_start+len(term)] if pos >= context_start else term,
                            f"**{term}**"
                        ) if pos >= context_start else fragment
    ...
    def extract_all_matches(file, terms, exact_string):
        ...
        # Extraer del contenido de texto - CON UNACCENT (ignora acentos)
        if hasattr(file, 'document') and file.document and file.document.text_content:
            full_text = file.document.text_content
            all_matches['text_content'] = extract_matches(full_text, terms, exact_string)
            for match in all_matches['text_content']:
        ...
        # Extraer de la descripción - CON UNACCENT (ignora acentos)
        if hasattr(file, 'document') and file.document and file.document.description:
            full_description = file.document.description
            all_matches['description'] = extract_matches(full_description, terms, exact_string)
            for match in all_matches['description']:
    ...
    for file in files:
        ...
        if hasattr(file, 'document') and file.document:
            ...
            if any(match['match_type'] == 'exact' for match in matched_texts.get('text_content', [])):
                matches_found.append('text_content_exact')
            elif any(match['match_type'] == 'term' for match in matched_texts.get('text_content', [])):
                matches_found.append('text_content')
            elif any(match['match_type'] == 'partial' for match in matched_texts.get('text_content', [])):
                matches_found.append('text_content_partial')
            elif has_search_vector_match(file, search_query):
                matches_found.append('search_vector')
            ...
            if any(match['match_type'] == 'exact' for match in matched_texts.get('description', [])):
                matches_found.append('description_exact')
            elif any(match['match_type'] == 'term' for match in matched_texts.get('description', [])):
                matches_found.append('description')
            elif any(match['match_type'] == 'partial' for match in matched_texts.get('description', [])):
                matches_found.append('description_partial')
            elif has_description_vector_match(file, search_query):
                matches_found.append('description_vector')
...
def search_google_drive_files_ranked_v9(search_string):
    """
    Búsqueda con ranking de relevancia y extracción de textos coincidentes
    1) Primero busca exactamente el string ingresado y da el ranking mayor
    2) Luego usa search_string.split() para búsqueda más amplia
    ...
    files = GoogleDriveFile.objects.filter(
        ...
        # Ranking para contenido de texto con UNACCENT (ignora acentos)
        content_unaccent_rank=Case(
            When(document__text_content__unaccent__icontains=search_string, then=Value(25.0)),
            default=Value(0.0),
            output_field=FloatField()
        ),
        # Ranking para descripción con UNACCENT (ignora acentos)
        description_unaccent_rank=Case(
            When(document__description__unaccent__icontains=search_string, then=Value(15.0)),
            default=Value(0.0),
            output_field=FloatField()
        ),
        # SearchRank para contenido de texto (vector)
        search_vector_rank=SearchRank(
            F('document__search_vector'), 
            search_query
        ),
        # SearchRank para descripción (vector)
        description_rank=SearchRank(
            F('document__description_vector'), 
            search_query
        ),
        ...
        total_rank=(
            ...
            (Coalesce('parent_rank', Value(0.0, output_field=FloatField())) * 1.0) +
            # -------- 3) CONTENIDO Y DESCRIPCIÓN (PESO BAJO) --------
            Coalesce('content_unaccent_rank', Value(0.0, output_field=FloatField())) * 0.5 +
            Coalesce('description_unaccent_rank', Value(0.0, output_field=FloatField())) * 0.3 +
            (Coalesce('search_vector_rank', Value(0.0, output_field=FloatField())) * 0.3) + 
            (Coalesce('description_rank', Value(0.0, output_field=FloatField())) * 0.2)
        )
    ).order_by('-total_rank')
    ...
    # Función auxiliar para extraer coincidencias de PALABRAS COMPLETAS - CON UNACCENT
    def extract_matches(text, terms, exact_string=None):
        """
        Extrae fragmentos de texto que coinciden con los términos de búsqueda
        Usa word boundaries (\\b) para encontrar palabras completas
        ...
        for term in terms:
            ...
            for match in re.finditer(pattern, text_normalized):
                ...
                if not is_duplicate and pos not in processed_positions:
                    ...
                    # Buscar el término en el texto original (con acentos)
                    original_term = text[pos-context_start:pos-context_start+len(term)] if pos >= context_start else term
                    
                    highlighted_fragment = fragment.replace(
                        original_term if pos >= context_start else term,
                        f"**{term}**"
                    ) if pos >= context_start else fragment
            ...
            for term in terms:
                ...
                if term_normalized in text_normalized:
                    ...
                    if pos not in processed_positions:
                        ...
                        # Resaltar el término en el contexto
                        term_in_fragment = text[pos-context_start:pos-context_start+len(term)] if pos >= context_start else term
                        highlighted_fragment = fragment.replace(
                            term_in_fragment if pos >= context_start else term,
                            f"**{term}**"
                        ) if pos >= context_start else fragment
    ...
    def extract_all_matches(file, terms, exact_string):
        """
        Extrae coincidencias de todos los campos del archivo
        Incluye búsqueda flexible en el nombre del archivo y contenido
        """
        all_matches = {
            'name': [],
            'parent_name': [],
    ...
    for file in files:
        ...
        result = {
            ...
            'relevance_details': {
                ...
                'parent_rank': float(file.parent_rank) if file.parent_rank else 0,
                # Contenido y descripción
                'content_unaccent_rank': float(file.content_unaccent_rank) if hasattr(file, 'content_unaccent_rank') else 0,
                'description_unaccent_rank': float(file.description_unaccent_rank) if hasattr(file, 'description_unaccent_rank') else 0,
                'search_vector_rank': float(file.search_vector_rank) if file.search_vector_rank else 0,
                'description_rank': float(file.description_rank) if file.description_rank else 0,
            },
            'matches_found_in': matches_found,
...
def search_google_drive_files_ranked(search_string):
    ...
    Incluye búsqueda flexible en el nombre del archivo (ignora acentos)
    Incluye búsqueda flexible en CONTENIDO y DESCRIPCIÓN (ignora acentos)
    RANKING: Mayor cantidad de TÉRMINOS DIFERENTES coincidentes
    """
    # Dividir la búsqueda en términos y filtrar palabras de menos de 4 caracteres
    raw_terms = search_string.strip().split()
    # Filtrar palabras con menos de 3 caracteres (excluir "a", "de", "los", etc.)
    search_terms = [term for term in raw_terms if len(term) >= 4]
    ...
    files = GoogleDriveFile.objects.filter(
        ...
        # ================================================================
        # RANKING TOTAL PONDERADO (SOLO PARA FILTRADO INICIAL)
        # ================================================================
        total_rank=(
            Coalesce('exact_name_rank', Value(0.0, output_field=FloatField())) * 5.0 +
            Coalesce('name_unaccent_rank', Value(0.0, output_field=FloatField())) * 4.5 +
    ...
    for file in files:
        ...
        # Calcular términos únicos coincidentes en Python (incluyendo CONTENIDO)
        name_term_count = 0
        parent_term_count = 0
        content_term_count = 0
        description_term_count = 0
        
        # Contar términos únicos en nombre
        if file.name:
            file_name_norm = normalize_text(file.name)
            for term in search_terms:
                term_norm = normalize_text(term)
                if term_norm in file_name_norm:
                    name_term_count += 1
        
        # Contar términos únicos en parent
        if file.parent_drive_file_id and file.parent_drive_file_id.name:
            parent_name_norm = normalize_text(file.parent_drive_file_id.name)
            for term in search_terms:
                term_norm = normalize_text(term)
                if term_norm in parent_name_norm:
                    parent_term_count += 1
        
        # Contar términos únicos en contenido (TEXT_CONTENT)
        if hasattr(file, 'document') and file.document and file.document.text_content:
            content_text_norm = normalize_text(file.document.text_content)
            for term in search_terms:
                term_norm = normalize_text(term)
                if term_norm in content_text_norm:
                    content_term_count += 1
        
        # Contar términos únicos en descripción
        if hasattr(file, 'document') and file.document and file.document.description:
            description_text_norm = normalize_text(file.document.description)
            for term in search_terms:
                term_norm = normalize_text(term)
                if term_norm in description_text_norm:
                    description_term_count += 1
        
        total_unique_terms = name_term_count + parent_term_count + content_term_count + description_term_count
        
        # Calcular relevance_score basado principalmente en términos únicos
        # Un documento con todos los términos tiene el máximo score
        max_possible_terms = len(search_terms) * 4  # 4 campos: nombre, parent, contenido, descripción
        if max_possible_terms > 0:
            # 70% del score basado en términos únicos, 30% en el ranking de la BD
            unique_term_score = (total_unique_terms / max_possible_terms) * 70
            db_rank_score = min((float(file.total_rank) / 100) * 30, 30) if file.total_rank else 0
            relevance_score = unique_term_score + db_rank_score
        else:
            relevance_score = float(file.total_rank) if file.total_rank else 0
        ...
        result = {
            ...
            'term_match_count': {
                'name_terms': name_term_count,
                'parent_terms': parent_term_count,
                'content_terms': content_term_count,
                'description_terms': description_term_count,
    ...
    # Ordenar resultados por relevance_score (prioriza términos únicos)
    results.sort(key=lambda x: x['relevance_score'], reverse=True)
